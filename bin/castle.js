...
cdb_jq_Server.prototype = {
	...
	,onMessage: function(msg) {
		...
		case 8:
			var id1 = msg[2];
			this.bindEvent(this.nodes[id1],id1,msg[3],msg[4]);
			break;
		case 9:
			var val = msg[4];
			var att = msg[3];
			var id2 = msg[2];
			if(val == null) {
				this.nodes[id2].removeAttribute(att);
			} else {
				this.nodes[id2].setAttribute(att,val);
			}
			break;
		case 10:
		...
		case 12:
			...
			var tmp = eid == null?function(_) {
				...
				_gthis.send(cdb_jq_Answer.Event(eid,{ value : v}));
			};
			this.handleSpecial(this.nodes[msg[2]],msg[3],msg[4],tmp);
			break;
		case 13:
...
Level.prototype = {
	...
	,init: function() {
		...
		while(key.hasNext()) this.watchSplit(key.next());
		...
		while(_g < _g1.length) {
			...
			++_g;
			var key1 = ld.l;
			var prev = __map_reserved[key1] != null?lprops.getReserved(key1):lprops.h[key1];
			if(prev != null) {
				HxOverrides.remove(this.props.layers,prev);
			}
			var key2 = ld.l;
			if(__map_reserved[key2] != null) {
				lprops.setReserved(key2,ld);
			} else {
				lprops.h[key2] = ld;
			}
		}
		...
		while(_g2 < _g11.length) {
				...
				if(tmp) {
					...
					while(_g31 < _g4.length) {
						...
						if(_g5[1] == 6) {
							...
							sid = (__map_reserved[rid] != null?_this.getReserved(rid):_this.h[rid]).s;
							idCol = cid.name;
							break;
						}
					}
			...
			case 12:
				...
				l2.loadSheetData((__map_reserved[type] != null?_this1.getReserved(type):_this1.h[type]).s);
				l2.setLayerData(val);
				this.layers.push(l2);
				break;
			case 15:
			...
			case 1:
				...
				if(l.images == null) {
					...
					while(_g1 < _g2) {
						var i2 = _g1++;
						var o = objs[0][i2];
						if(x >= o.x && y >= o.y && x < o.x + (l.hasSize?o.width:1) && y < o.y + (l.hasSize?o.height:1)) {
							if(l.idToIndex == null) {
					...
					while(_g11 < _g3) {
						...
						var img = l.images[k1];
						var w = img.width / this.tileSize;
						var h = img.height / this.tileSize;
						var ox = o3.x - (w - 1) * 0.5;
						var oy = o3.y - (h - 1);
						if(x >= ox && y >= oy && x < ox + w && y < oy + h && !this.hasHole(img,(x - ox) * this.tileSize | 0,(y - oy) * this.tileSize | 0)) {
	...
	,popupLayer: function(l,mouseX,mouseY) {
		...
		ndel.click = function() {
			HxOverrides.remove(Reflect.field(_gthis.obj,l.listColumnn.name),l.targetObj.o);
			_gthis.save();
			_gthis.reload();
		};
		...
		while(_g < _g1.length) {
			...
			case 1:
				...
				while(_g21 < _g31.length) {
					var o = _g31[_g21];
					++_g21;
					if(sx + sw <= o.x || sy + sh <= o.y || sx >= o.x + (l.hasSize?o.width:1) || sy >= o.y + (l.hasSize?o.height:1)) {
						continue;
					}
			...
			case 3:
				...
				while(_g22 < _g33.length) {
					...
					++_g22;
					var o1 = objs1.h[i.o];
					if(sx + sw <= i.x || sy + sh <= i.y || sx >= i.x + (o1 == null?1:o1.w) || sy >= i.y + (o1 == null?1:o1.h)) {
						continue;
					}
		...
		while(_g < _g1.length) {
			...
			case 3:
				...
				while(_g26 < _g35.length) {
					...
					++_g26;
					var o1 = objs1.h[i2.o];
					if(sx + sw <= i2.x || sy + sh <= i2.y || sx >= i2.x + (o1 == null?1:o1.w) || sy >= i2.y + (o1 == null?1:o1.h)) {
						continue;
					}
	...
	,updateCursorPos: function() {
		...
		if(this.selection != null) {
			...
			this.selection.x = x0;
			this.selection.y = y0;
			this.selection.w = (ccx1 < this.selection.sx?this.selection.sx:ccx1) - x0;
			this.selection.h = (ccy1 < this.selection.sy?this.selection.sy:ccy1) - y0;
			if(!fc1) {
				this.selection.w += 1;
				this.selection.h += 1;
			}
	...
	,onKey: function(e) {
		...
		case 9:
			...
			this.setLayer(this.layers[(i + (e.shiftKey?this.layers.length - 1:1)) % this.layers.length]);
			e.preventDefault();
			e.stopPropagation();
			break;
		case 27:
	...
	,setLayer: function(l) {
		...
		tmp1.spectrum("set",this.toColor(l.props.color)).closest(".item").css({ display : css});
		var _g1 = l.data;
		switch(_g1[1]) {
		case 2:
			this.content.find("[name=size]").val("" + _g1[2].size).closest(".item").show();
...
Model.prototype = {
	...
	,valToString: function(t,val,esc) {
		...
		case 2:case 3:case 4:case 7:
			return Std.string(val);
		case 9:
			return this.typeValToString(this.tmap.get(t[2]),val,esc);
		case 10:
			var values = t[2];
			var v = val;
			var flags = [];
	...
	,parseVal: function(t,val) {
		...
		case 6:
			...
			return r.id;
		case 9:
			return this.parseTypeVal(this.tmap.get(t[2]),val);
		case 11:
			if(val.charAt(0) == "#") {
		...
		convertTypeRec = function(t,o) {
			...
			while(_g1 < _g) {
				...
				case 6:
					...
					break;
				case 9:
					var convertTypeRec1 = _gthis.tmap.get(_g2[2]);
		...
		convertTypeRec = function(t1,v3) {
			...
			while(_g13 < _g4) {
				...
				if(_g21[1] == 9) {
					var av = v3[i1 + 1];
					if(av != null) {
						var convertTypeRec1 = _gthis.tmap.get(_g21[2]);
...
Main.prototype = $extend(Model.prototype,{
	...
	,changed: function(sheet,c,index,old) {
		...
		case 14:
			var oldV = old;
			var newV = Reflect.field(sheet.lines[index],c.name);
	...
	,valueHtml: function(c,v,sheet,obj) {
		...
		case 6:
			if(v == "") {
				...
				var i = this.smap.get(_g[2]).index.get(v);
				if(i == null) {
		...
		case 9:
			var a1 = v;
			var cas = this.tmap.get(_g[2]).cases[a1[0]];
			var str = cas.name;
	...
	,editCell: function(c,v,sheet,index) {
		...
		case 0:case 1:case 3:case 4:case 9:case 16:
			...
			if(val != null) {
				...
				case 9:
					var t = _g5[2];
					var tmp1 = this.typeValToString(this.tmap.get(t),val);
					i4.val(tmp1);
					break;
				case 16:
					...
					break;
				default:
					var tmp3 = "" + Std.string(val);
					i4.val(tmp3);
				}
			}
			i4.change(function(e5) {
				e5.stopPropagation();
			});
			i4.keydown(function(e6) {
				var _g6 = e6.keyCode;
				switch(_g6) {
				case 9:
					i4.blur();
		...
		case 11:
			...
			spect.spectrum({ color : "#" + StringTools.hex(val,6), showInput : true, showButtons : false, change : function() {
				...
				val = color;
				obj[c.name] = color;
				var tmp4 = _gthis.valueHtml(c,val,sheet,obj);
				v.html(tmp4);
				_gthis.save();
			}}).spectrum("show");
			break;
		...
		while(_g31 < _g22) {
			...
			while(_g5 < _g4) {
				...
				case 8:
					...
					var tmp2 = (function(key1,html2,l2,v3,val3,obj3,index4,c7,cindex2) {
						return function(e7) {
							...
							var tmp3 = (function(key2,html3,v4,val4,obj4,c8) {
								return function(e8) {
				...
				case 13:
					...
					var tmp6 = (function(set2) {
						return function(_3) {
							var tmp7 = (function(set3) {
								return function(path) {
									set3[0](path);
									_gthis.save();
								};
							})(set2);
							_gthis.chooseFile(tmp7);
						};
					})(set);
					v[0].dblclick(tmp6);
					break;
				case 14:
					...
					var tmp8 = (function(set4,v6,val5,index5,c11) {
						return function(_4) {
							var rv = val5[0];
							...
							dialog.keydown(tmp11).keypress((function() {
								return function(e13) {
									e13.stopPropagation();
								};
							})());
				...
				case 17:
					...
					var tmp12 = (function(key4,html4,l3,v11,val6,obj7,index6,c12,cindex3) {
						return function(e14) {
							...
							var tmp13 = (function(key5,html5,v12,val7,obj8,c13) {
								return function(e15) {
					...
					break;
				default:
					var tmp16 = (function(v14,index7,c14) {
						return function(e16) {
			...
			while(sheet.separators[snext] == i5) {
				...
				sep.dblclick((function(pos,title1,content4) {
					return function(e17) {
						content4[0].empty();
						var tmp17 = $("<input>");
						var tmp18 = title1[0] == null?"":title1[0];
						tmp17.appendTo(content4[0]).focus().val(tmp18).blur((function(pos1,title2,content5) {
							return function(_14) {
								var tmp19 = $(this).val();
								title2[0] = tmp19;
								$(this).remove();
						...
						return function(_2) {
							...
							while(_g2 < _g3.length) {
								...
								if(StringTools.startsWith(s4.name,old + "@")) {
									var tmp1 = HxOverrides.substr(s4.name,old.length + 1,null);
									s4.name = name + "@" + tmp1;
								}
							}
							_gthis.initContent();
...
cdb_BinSerializer.checkSchemasData = function(b) {
	...
	while(pos < b.length) {
		...
		hash |= b.b[pos++] << 16;
		hash |= b.b[pos++] << 24;
		var s = cdb_BinSerializer.schemas.iterator();
		while(s.hasNext()) {
			var s1 = s.next();
			if(s1.name == name) {
				if(s1.hash != hash) {
...
cdb_BinSerializer.getSchemasData = function() {
	...
	while(s.hasNext()) {
		var s1 = s.next();
		b.b.push(s1.name.length);
		var src = haxe_io_Bytes.ofString(s1.name);
		var b2 = src.b;
		var _g1 = 0;
		var _g = src.length;
		while(_g1 < _g) b.b.push(b2[_g1++]);
		b.b.push(s1.hash & 255);
		b.b.push(s1.hash >> 8 & 255);
		b.b.push(s1.hash >> 16 & 255);
		b.b.push(s1.hash >>> 24);
	}
	return b.getBytes();
};
cdb_BinSerializer.prototype = {
	...
	,serializeData: function(d,v) {
		...
		case 7:
			...
			while(_g4 < a.length) {
				var v1 = a[_g4];
				++_g4;
				this.serializeData(d1,v1);
			}
			break;
		case 8:
			this.serializeSchema(cdb_BinSerializer.schemas.get(d[2]),v);
			break;
		case 9:
			var fields = d[2];
			var _g5 = 0;
		...
		case 11:
			...
			while(k2.hasNext()) {
				var k3 = k2.next();
				this.serializeInt(k3.length);
				var _this3 = this.out;
				var src2 = haxe_io_Bytes.ofString(k3);
				var b23 = src2.b;
				var _g13 = 0;
				var _g6 = src2.length;
				while(_g13 < _g6) _this3.b.push(b23[_g13++]);
				this.serializeData(d3,__map_reserved[k3] != null?m1.getReserved(k3):m1.h[k3]);
			}
			this.out.b.push(255);
			break;
		case 12:
			...
			while(k4.hasNext()) {
				var k5 = k4.next();
				this.serializeSchema(s1,k5);
				this.serializeData(d4,m2.get(k5));
			}
			this.out.b.push(255);
			break;
		}
	...
	,unserializeData: function(d) {
		...
		case 6:
			...
			return this.unserializeData(d[2]);
		case 7:
			var d1 = d[2];
			...
			while(_g2 < len3) {
				++_g2;
				_g.push(this.unserializeData(d1));
			}
			return _g;
		case 8:
			return this.unserializeSchema(cdb_BinSerializer.schemas.get(d[2]));
		case 9:
			var fields = d[2];
			var o = { };
			var _g1 = 0;
		...
		case 10:
			...
			while(this.bytes.b[this.position++] != 255) {
				...
				var value = this.unserializeData(d2);
				m.h[key] = value;
			}
			return m;
		case 11:
			...
			while(this.bytes.b[this.position++] != 255) {
				...
				this.position += len4;
				var value1 = this.unserializeData(d3);
				if(__map_reserved[key1] != null) {
		...
		case 12:
			...
			while(this.bytes.b[this.position++] != 255) {
				this.position--;
				m2.set(this.unserializeSchema(s),this.unserializeData(d4));
			}
			return m2;
		}
	}
		...
		while(_g < _g1.length) {
			...
			case 7:
				...
				while(_g3 < _g2) {
					...
					var idat = this.level.model.getImageData(Reflect.field(sheet.lines[idx[0]],c.name));
					if(idat == null) {
...
lvl_LayerData.prototype = $extend(lvl_LayerGfx.prototype,{
	...
	,setLayerData: function(val) {
		if(val == null || val == "") {
			...
			while(_g21 < _g11) _g3.push(a.b[_g21++]);
			...
			if(_g3 == null) {
				...
				case "objects":
					...
					while(p < data.length) {
						...
						insts.push({ x : x1, y : y1, o : v22, flip : flip, rot : x >> 15 | y >> 15 << 1});
					}
		...
		case "ground":case "tiles":
			...
			case "objects":
				...
				if(_g[1] == 2) {
					...
					this.data = lvl_LayerInnerData.TileInstances(td,_g2);
					this.dirty = true;
				} else {
		...
		case "objects":
			...
			case "ground":case "tiles":
				...
				if(_g8[1] == 3) {
					...
					this.data = lvl_LayerInnerData.Tiles(td1,_g9);
					this.dirty = true;
				} else {
		...
		case 2:
			...
			if(this.props.mode == "ground") {
				...
				while(p < max) {
					var x2 = p++;
					var y2 = p++;
					view.draw(this.images[a[p++]],a[x2] * size,a[y2] * size);
				}
			}
			break;
		case 3:
	...
	init: function() {
		...
		while(_g2 < _g11.length) {
			...
			if(_g21[1] == 6) {
				...
				var _this = this.level.model.smap;
				g.fromSheet((__map_reserved[s] != null?_this.getReserved(s):_this.h[s]).s,16711680);
	...
	,layerChanged: function(l) {
		...
		jsel.mousedown(function(e2) {
			...
			if(e2.shiftKey) {
				...
				l.currentWidth = (x1 < start_x?start_x:x1) - x0 + 1;
				l.currentHeight = (y1 < start_y?start_y:y1) - y0 + 1;
				l.saveState();
		...
		jsel.mousemove(function(e3) {
			...
			l.currentWidth = (x2 < start_x?start_x:x2) - x01 + 1;
			l.currentHeight = (y2 < start_y?start_y:y2) - y01 + 1;
			l.saveState();
			_gthis.level.setCursor();
		});
