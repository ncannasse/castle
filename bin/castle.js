	...
	,editCell: function(c,v,sheet,index) {
			...
			i.blur(function(_) {
				...
				if(c.type == cdb_ColumnType.TId && prevObj != null && old1 != val) {
					...
					if(prevObj.obj == obj) {
						...
						tmp4 = (__map_reserved[old1] != null ? _this2.getReserved(old1) : _this2.h[old1]) != null;
					} else {
						tmp4 = false;
					}
					if(!tmp4) {
						if(prevTarget != null) {
							...
							var key3 = val;
							tmp3 = (__map_reserved[key3] != null ? _this3.getReserved(key3) : _this3.h[key3]).obj != prevTarget.obj;
						} else {
							tmp3 = false;
						}
					} else {
		...
		case 5:
			...
			while(_g11 < _g5) {
				var i1 = _g11++;
				var tmp5 = $("<option>");
				var tmp6 = val == i1 ? "selected" : "_sel";
				tmp5.attr("value","" + i1).attr(tmp6,"selected").text(values[i1]).appendTo(s);
			}
			if(c.opt) {
		...
		case 11:
			...
			spect.spectrum({ color : "#" + StringTools.hex(val,6), showInput : true, showButtons : false, change : function() {
				...
				val = color;
				obj[c.name] = color;
				var tmp7 = _gthis.valueHtml(c,val,sheet,obj);
				v.html(tmp7);
				_gthis.save();
			}}).spectrum("show");
			break;
...
hxbit_enumSer_hxbit_$PropTypeDesc.getSchema = function() {
	...
	var _g51 = 0;
	var v5;
	var _g6 = [hxbit_PropTypeDesc.PArray(hxbit_PropTypeDesc.PObj([{ name : "name", opt : false, type : hxbit_PropTypeDesc.PString},{ name : "opt", opt : false, type : hxbit_PropTypeDesc.PBool},{ name : "type", opt : false, type : hxbit_PropTypeDesc.PEnum("hxbit.PropTypeDesc")}]))];
...
hxbit_enumSer_vdom_$Message.getSchema = function() {
	...
	var v1;
	var v2;
	var _g2 = [hxbit_PropTypeDesc.PArray(hxbit_PropTypeDesc.PObj([{ name : "name", opt : false, type : hxbit_PropTypeDesc.PString},{ name : "value", opt : false, type : hxbit_PropTypeDesc.PString}]))];
	while(_g1 < _g2.length) {
		var t = _g2[_g1];
		++_g1;
		_g.push({ name : "", type : t, opt : false});
