...
Level.prototype = {
	...
	,getTileProps: function(file,stride) {
		...
		var p = Reflect.field(this.props.tileSets,file);
		if(p == null) {
			p = { stride : stride, sets : [], tags : []};
			this.props.tileSets[file] = p;
		} else {
			...
			if(p.stride == null) p.stride = stride; else if(p.stride != stride) {
				...
				while(_g < _g1.length) {
					...
					var out = [];
					var _g3 = 0;
					var _g2 = Math.ceil(t.flags.length / p.stride);
					while(_g3 < _g2) {
						var y = _g3++;
						var _g5 = 0;
						var _g4 = p.stride;
						while(_g5 < _g4) {
							var x = _g5++;
							if(t.flags[x + y * p.stride]) out[x + y * stride] = true;
						}
					}
					t.flags = out;
				}
			...
			if(!l.visible) continue;
			{
				...
				case 3:
					...
					while(_g21 < _g11) {
						...
						if(this.curPos.xf >= i2.x && this.curPos.yf >= i2.y && this.curPos.xf < i2.x + (o1 == null?1:o1.w) && this.curPos.yf < i2.y + (o1 == null?1:o1.h)) return { k : i2.o, layer : l, index : idx2};
					}
					break;
				}
			}
			...
			}
			{
				...
				case 1:
					...
					while(_g27 < _g16) {
						...
						var o = objs1[i6];
						if(o.x == px && o.y == py) {
							_g3.editProps(_g3.currentLayer,i6);
							return;
						}
					}
					var o1 = { x : px, y : py};
	...
	,onKey: function(e) {
		...
		if(e.ctrlKey || new js.JQuery(":focus").length > 0) return;
		new js.JQuery(".popup").remove();
		var _g = e.keyCode;
		switch(_g) {
		case 107:
	...
	,set: function(x,y) {
		...
		var l = this.currentLayer;
		{
			...
			case 3:
				...
				if(putObj != null) insts.push({ x : x1, y : y1, o : l.current}); else {
					var _g21 = 0;
	...
	,savePrefs: function() {
		var sc = this.content.find(".scroll");
		var state = { zoomView : this.zoomView, curLayer : this.currentLayer == null?null:this.currentLayer.name, scrollX : sc.scrollLeft(), scrollY : sc.scrollTop(), paintMode : this.paintMode, randomMode : this.randomMode, tagMode : this.tagMode};
	...
	,paletteOption: function(name,val) {
		...
		case "mode":
			var s = l.getTileProp();
			var m = cdb._Data.TileMode_Impl_.ofString(val);
			if(s == null) {
				if(m == "tile") return;
			...
			} else if(m == "tile") HxOverrides.remove(l.tileProps.sets,s); else {
				if(s.t == m) return;
				s.t = m;
				s.opts = { };
			}
		...
		case "border":
			...
			if(s3 != null) {
				if(val == "null") Reflect.deleteField(s3.opts,"border"); else s3.opts.border = val;
			}
			break;
		case "border_out":
			var s4 = l.getTileProp();
			if(s4 != null) {
				if(val != "true") Reflect.deleteField(s4.opts,"borderOut"); else s4.opts.borderOut = true;
			}
			break;
		case "tag":
		...
		case "addTag":
			if(val == "") return;
			var _g2 = 0;
			var _g11 = l.tileProps.tags;
			while(_g2 < _g11.length) {
				var t = _g11[_g2];
				++_g2;
				if(t.name == val) return;
			}
		...
		case "delTag":
			var _g3 = 0;
			var _g12 = l.tileProps.tags;
			while(_g3 < _g12.length) {
				var t1 = _g12[_g3];
				++_g3;
				if(t1.name == this.tagMode) {
					HxOverrides.remove(l.tileProps.tags,t1);
					var t2 = l.tileProps.tags[0];
					if(t2 == null) this.tagMode = ""; else this.tagMode = t2.name;
					this.savePrefs();
				}
			}
			this.setCursor(l);
	...
	,setCursor: function(l) {
		...
		if(old != l) {
			...
			if(l.images != null) {
				...
				jsel.mousedown(function(e) {
					...
					if(_g.tagMode != null) {
						var t1 = Lambda.find(l.tileProps.tags,function(t2) {
							return t2.name == _g.tagMode;
						});
						if(t1 != null) {
		...
		if(this.paletteSelect != null) {
			...
			if(l.tileProps == null) m.hide(); else if(this.tagMode != null) {
				t4.find("[name=tags]").html(((function($this) {
					var $r;
					var _g7 = [];
					{
						var _g18 = 0;
						var _g23 = l.tileProps.tags;
						while(_g18 < _g23.length) {
							var t5 = _g23[_g18];
							++_g18;
							_g7.push("<option value=\"" + t5.name + "\">" + t5.name + "</option>");
						}
					}
					$r = _g7;
					return $r;
				}(this))).join("")).val(this.tagMode);
				m.hide();
				t4.show();
				var t6 = Lambda.find(l.tileProps.tags,function(t7) {
					return t7.name == _g.tagMode;
				});
				if(t6 != null) {
					var _g24 = 0;
					var _g19 = this.height;
					while(_g24 < _g19) {
						var y3 = _g24++;
						var _g42 = 0;
						var _g32 = this.width;
						while(_g42 < _g32) {
							var x3 = _g42++;
							if(t6.flags[x3 + y3 * l.imagesStride]) this.paletteSelect.fillRect(x3 * (this.tileSize + 1),y3 * (this.tileSize + 1),this.tileSize + 1,this.tileSize + 1,-2131010655);
						}
					}
				}
			} else {
				...
				while(_g8 < _g110.length) {
					...
					var py = s.y * (this.tileSize + 1);
					var w = s.w * (this.tileSize + 1) - 1;
					var h = s.h * (this.tileSize + 1) - 1;
				...
				var tkind = cdb._Data.TileMode_Impl_.toString(tobj.t);
				...
				case "object":
					break;
				case "border":
					var opts = "<option value='null'>any</option>" + ((function($this) {
						var $r;
						var _g111 = [];
						{
							var _g26 = 0;
							while(_g26 < grounds.length) {
								var g = grounds[_g26];
								++_g26;
								_g111.push("<option value=\"" + g + "\">" + g + "</option>");
							}
						}
						$r = _g111;
						return $r;
		...
		if(l.images != null) {
			...
			while(_g10 < h1) {
				...
				while(_g112 < w1) {
					var x4 = _g112++;
					var i3 = l.images[l.current + x4 + y4 * l.imagesStride];
...
cdb.TileBuilder = function(t,stride,total) {
		...
		case "ground":
			...
			while(_g5 < _g41) {
				...
				while(_g7 < _g6) {
					...
					bl[15].push(tid - 1);
					this.grounds[tid] = gid;
				}
			}
			this.groundIds.set(s.opts.name,gid);
			break;
		case "object":
			break;
		case "border":
	...
	while(_g8 < _g12.length) {
		...
		++_g8;
		if(s1.t == "border") {
			var gid1 = this.groundIds.get(s1.opts.border);
			if(gid1 == null) continue;
			var bt = this.borders[gid1];
			if(bt == null) continue;
			var _g31 = 0;
			var _g22 = s1.w;
			while(_g31 < _g22) {
				...
				while(_g51 < _g42) {
					var dy1 = _g51++;
					var k;
					if(s1.opts.borderOut) {
						if(dx1 == 0 && dy1 == 0) k = 14; else if(dx1 == s1.w - 1 && dy1 == 0) k = 13; else if(dx1 == 0 && dy1 == s1.h - 1) k = 11; else if(dx1 == s1.w - 1 && dy1 == s1.h - 1) k = 7; else continue;
					} else if(dy1 == 0) if(dx1 == 0) k = 1; else if(dx1 == s1.w - 1) k = 2; else k = 3; else if(dy1 == s1.h - 1) if(dx1 == 0) k = 4; else if(dx1 == s1.w - 1) k = 8; else k = 12; else if(dx1 == 0) k = 5; else if(dx1 == s1.w - 1) k = 10; else continue;
					bt[k].push(s1.x + dx1 + (s1.y + dy1) * stride);
				}
			}
		}
	}
	...
	,buildGrounds: function(input,width) {
		...
		while(_g < height) {
			...
			while(_g1 < width) {
				var x = _g1++;
				var v = input[++p];
				var g = this.grounds[v];
				var gl;
				if(x == 0) gl = g; else gl = this.grounds[input[p - 1]];
				var gr;
				if(x == width - 1) gr = g; else gr = this.grounds[input[p + 1]];
				var gt;
				if(y == 0) gt = g; else gt = this.grounds[input[p - width]];
				var gd;
				if(y == height - 1) gd = g; else gd = this.grounds[input[p + width]];
				var gtl;
				if(x == 0 || y == 0) gtl = g; else gtl = this.grounds[input[p - 1 - width]];
				var gtr;
				if(x == width - 1 || y == 0) gtr = g; else gtr = this.grounds[input[p + 1 - width]];
				var gbl;
				if(x == 0 || y == height - 1) gbl = g; else gbl = this.grounds[input[p - 1 + width]];
				var gbr;
				if(x == width - 1 || y == height - 1) gbr = g; else gbr = this.grounds[input[p + 1 + width]];
				var max;
				var a;
				var a1;
				if(gr > gl) a1 = gr; else a1 = gl;
				var b;
				if(gt > gd) b = gt; else b = gd;
				...
				if(a5 <= g) a5 = 1000;
				if(b3 <= g) b3 = 1000;
				if(a5 > b3) a4 = b3; else a4 = a5;
				var b4;
				var a6 = gt;
				var b5 = gd;
				if(a6 <= g) a6 = 1000;
				if(b5 <= g) b5 = 1000;
				if(a6 > b5) b4 = b5; else b4 = a6;
				if(a4 <= g) a4 = 1000;
				if(b4 <= g) b4 = 1000;
				...
				if(a8 <= g) a8 = 1000;
				if(b7 <= g) b7 = 1000;
				if(a8 > b7) a7 = b7; else a7 = a8;
				var b8;
				var a9 = gbr;
				var b9 = gbl;
				if(a9 <= g) a9 = 1000;
				if(b9 <= g) b9 = 1000;
				if(a9 > b9) b8 = b9; else b8 = a9;
				if(a7 <= g) a7 = 1000;
				if(b8 <= g) b8 = 1000;
				if(a7 > b8) b6 = b8; else b6 = a7;
				if(a3 <= g) a3 = 1000;
				if(b6 <= g) b6 = 1000;
				if(a3 > b6) min = b6; else min = a3;
				var _g2 = min - 1;
				while(_g2 < max) {
					var g1 = _g2++;
					var bits = 0;
					...
					if(a10.length == 0) {
						...
						case 6:
							...
							if(a10.length > 0) {
								out.push(x);
								out.push(y);
								out.push(a10.length == 1?a10[0]:a10[this.random(x + y * width) % a10.length]);
							}
							a10 = bb[4];
							break;
						case 9:
