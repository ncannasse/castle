...
cdb_jq_Server.prototype = {
	...
	,bindEvent: function(n,id,name,eid) {
		...
		var callb = function(e) {
			...
			if(sendValue) {
				_g.send(cdb_jq_Answer.SetValue(id,"" + Std.string(Reflect.field(n,"value"))));
			}
	...
	,onMessage: function(msg) {
		...
		case 15:
			...
			while(_g2 < eids.length) {
				var eid2 = eids[_g2];
				++_g2;
				var e1 = this.events.h[eid2];
				if(e1 != null) {
					this.events.remove(eid2);
					e1.n.removeEventListener(e1.name,e1.callb);
				}
			}
			break;
		case 14:
			...
			this.nodes[id2] = null;
			if(eids1 != null) {
				this.onMessage(cdb_jq_Message.Unbind(eids1));
			}
			break;
		case 16:
			var msgs = msg[2];
			var _g3 = 0;
			while(_g3 < msgs.length) {
				var m1 = msgs[_g3];
				++_g3;
				this.onMessage(m1);
			}
...
JqPage.prototype = $extend(cdb_jq_Server.prototype,{
	...
	,bindEvent: function(n,id,name,eid) {
		...
		case "paneldock":
			...
			p.__onDestroy = function() {
				_g.send(cdb_jq_Answer.Event(eid,{ }));
			};
			break;
		default:
	...
	,handleSpecial: function(e,name,args,result) {
		...
		case "dock":
			...
			var n1 = this.dnodes.h[parent.__id__];
			if(n1 == null) {
			...
			var _g3 = dir.toLowerCase();
			var n2;
			switch(_g3) {
			case "left":
				n2 = this.dockManager.dockLeft(n1,p1,size1);
				break;
			case "right":
				n2 = this.dockManager.dockRight(n1,p1,size1);
				break;
			case "up":
				n2 = this.dockManager.dockUp(n1,p1,size1);
				break;
			case "down":
				n2 = this.dockManager.dockDown(n1,p1,size1);
				break;
			default:
				n2 = this.dockManager.dockFill(n1,p1);
			}
			this.dnodes.set(e,n2);
			break;
		case "scrollIntoView":
			e.scrollIntoView();
			break;
...
Level.prototype = {
	...
	,init: function() {
		...
		while(_g2 < _g12.length) {
			...
			case 12:
				...
				l.loadSheetData((__map_reserved[type] != null?_this.getReserved(type):_this.h[type]).s);
				l.setLayerData(val);
				this.layers.push(l);
				break;
			case 8:
				...
				var sheet = (__map_reserved[key2] != null?_this1.getReserved(key2):_this1.h[key2]).s;
					...
					try {
						while(_g3 < _g4.length) {
							...
							case 6:
								...
								sid = (__map_reserved[rid] != null?_this2.getReserved(rid):_this2.h[rid]).s;
								idCol = cid.name;
								throw "__break__";
								break;
							default:
	...
	,setup: function() {
			...
			td.mousedown((function(l1) {
				return function(e) {
					...
					case 1:
						...
						break;
					case 3:
						_g3.popupLayer(l1[0],e.pageX | 0,e.pageY | 0);
						e.preventDefault();
						break;
					}
				};
		...
		var callb = this.allocRef(function(_1) {
			...
			while(_g22 < _g11) {
				var i2 = _g22++;
				_g5.push(_g3.layers[indexes[i2]]);
			}
			_g3.layers = _g5;
			var _g23 = 0;
			...
			while(_g23 < _g12) {
				var i3 = _g23++;
				_g3.layers[i3].comp.data("index",i3);
			}
		...
		scroll[0].onmousewheel = function(e1) {
			if(e1.shiftKey) {
				_g3.updateZoom(e1.wheelDelta > 0);
			}
		};
		...
		var onMouseUp = function(_4) {
			_g3.mouseDown = null;
			if(_g3.currentLayer != null && _g3.currentLayer.hasSize) {
				_g3.setCursor();
			}
			if(_g3.needSave) {
				_g3.save();
		...
		scroll.mousedown(function(e3) {
			...
			case 1:
				...
				var w = o == null?_g3.currentLayer.currentWidth:o.w;
				var h = o == null?_g3.currentLayer.currentHeight:o.h;
				if(o == null && _g3.palette.randomMode) {
					h = 1;
					w = h;
				}
		...
		this.content.mouseup(function(e4) {
			...
			if(e4.which == 1 && _g3.selection == null && _g3.currentLayer.enabled() && _g3.curPos.x >= 0 && _g3.curPos.y >= 0) {
				_g3.setObject();
			}
			_g3.startPos = null;
			if(_g3.selection != null) {
		...
		while(_g < _g1.length) {
			...
			case 0:
				...
				while(_g4 < sw1) {
					var dx = _g4++;
					var _g6 = 0;
					while(_g6 < sh1) data[sx1 + dx + (sy1 + _g6++) * this.width] = 0;
				}
				break;
			case 2:
				var data1 = _g2[3];
				...
				while(_g41 < sw2) {
					var dx1 = _g41++;
					var _g61 = 0;
					while(_g61 < sh2) data1[sx2 + dx1 + (sy2 + _g61++) * this.width] = 0;
				}
				break;
			case 3:
				var insts = _g2[3];
				...
				while(_g3 < _g42.length) {
					var i = _g42[_g3];
					++_g3;
					var o = objs.h[i.o];
					var ow = o == null?1:o.w;
					var oh = o == null?1:o.h;
					if(sx + sw <= i.x || sy + sh <= i.y || sx >= i.x + ow || sy >= i.y + oh) {
						continue;
					}
			...
			case 1:
				...
				while(_g31 < _g43.length) {
					var o1 = _g43[_g31];
					++_g31;
					var ow1 = l.hasSize?o1.width:1;
					var oh1 = l.hasSize?o1.height:1;
					if(sx + sw <= o1.x || sy + sh <= o1.y || sx >= o1.x + ow1 || sy >= o1.y + oh1) {
						continue;
					}
		...
		while(_g < _g1.length) {
			...
			case 3:
				...
				while(_g34 < _g44.length) {
					...
					var ow = o == null?1:o.w;
					var oh = o == null?1:o.h;
					if(sx + sw <= i2.x || sy + sh <= i2.y || sx >= i2.x + ow || sy >= i2.y + oh) {
						continue;
					}
			...
			case 1:
				...
				while(_g35 < _g45.length) {
					...
					o1.x += l.hasFloatCoord?dx:ix;
					o1.y += l.hasFloatCoord?dy:iy;
					if(o1.x < 0 || o1.y < 0 || o1.x >= this.width || o1.y >= this.height) {
						HxOverrides.remove(objs1,o1);
					}
				}
				break;
	...
	,editProps: function(l,index) {
		...
		while(_g1 < _g11.length) {
			...
			td[0].click((function(td1,c1) {
				return function(e3) {
					main.editCell(c1[0],td1[0],{ columns : l.baseSheet.columns, props : l.baseSheet.props, name : l.baseSheet.name, path : SheetData.getPath(l.baseSheet) + ":" + index, parent : { sheet : _g.sheet, column : Lambda.indexOf(_g.sheet.columns,Lambda.find(_g.sheet.columns,(function() {
						return function(c2) {
							return c2.name == l.name;
						};
					})())), line : index}, lines : Reflect.field(_g.obj,l.name), separators : []},index);
					e3.preventDefault();
					e3.stopPropagation();
				};
			})(td,c));
	...
	,onKey: function(e) {
		...
		case 39:
			...
			if(l.current % l.stride < l.stride - w1 && l.images != null && l.current + w1 < l.images.length) {
				l.set_current(l.current + w1);
				if(w1 != 1 || h1 != 1) {
					l.currentWidth = w1;
					l.currentHeight = h1;
					l.saveState();
		...
		case 40:
			...
			if(l.images != null && l.current + l.stride * h2 < l.images.length) {
				l.set_current(l.current + l.stride * h2);
				if(w2 != 1 || h2 != 1) {
					l.currentWidth = w2;
					l.currentHeight = h2;
					l.saveState();
	...
	,doDelete: function() {
		...
		case 2:
			...
			while(_g2 < _g11) {
				...
				var _g3 = w;
				while(_g4 < _g3) {
					var i = p.index + _g4++ + dy * this.width;
					if(data1[i] == 0) {
						continue;
					}
					data1[i] = 0;
	...
	,set: function(x,y,replace) {
		...
		case 3:
			...
			insts.sort(function(i11,i21) {
				var o1 = objs.h[i11.o];
				var tmp = (i11.y + (o1 == null?1:o1.h)) * _g1.tileSize | 0;
				var o2 = objs.h[i21.o];
				var dy4 = tmp - ((i21.y + (o2 == null?1:o2.h)) * _g1.tileSize | 0);
				if(dy4 == 0) {
					var o3 = objs.h[i11.o];
					var tmp1 = (i11.x + (o3 == null?0.5:o3.w * 0.5)) * _g1.tileSize | 0;
					var o4 = objs.h[i21.o];
					return tmp1 - ((i21.x + (o4 == null?0.5:o4.w * 0.5)) * _g1.tileSize | 0);
				} else {
					return dy4;
				}
			});
...
Model.prototype = {
			...
			while(_g2 < _g3.length) {
				...
				case 12:
					...
					while(_g5 < _g6.length) {
						...
						var _g7 = [];
						var _g8 = 0;
						while(_g8 < 256) _g7.push(_g8++);
						ldat = cdb__$Types_Layer_$Impl_$.encode(cdb__$Types_Layer_$Impl_$.decode(ldat,_g7),this.data.compress);
						obj[c1.name] = ldat;
					}
					break;
				case 15:
	...
	,parseVal: function(t,val) {
		...
		case 9:
			var t1 = t[2];
			var _this = this.tmap;
			return this.parseTypeVal(__map_reserved[t1] != null?_this.getReserved(t1):_this.h[t1],val);
		case 6:
			var t2 = t[2];
			var _this1 = this.smap;
			var r = (__map_reserved[t2] != null?_this1.getReserved(t2):_this1.h[t2]).index.get(val);
			if(r == null) {
				throw new js__$Boot_HaxeError(val + " is not a known " + t2 + " id");
			}
			return r.id;
		case 11:
			if(val.charAt(0) == "#") {
...
Main.prototype = $extend(Model.prototype,{
			...
			if(inCDB) {
				...
				if(this.cursor.s != null) {
					if(this.cursor.s.props.isProps) {
						...
						var _g1 = s1.y2 + 1;
						while(_g2 < _g1) {
							var obj = this.cursor.s.lines[_g2++];
							var _g4 = s1.x1;
							var _g3 = s1.x2 + 1;
							while(_g4 < _g3) {
								var c = this.cursor.s.columns[_g4++];
		...
		case 67:
			if(e.ctrlKey) {
				if(this.cursor.s != null) {
					...
					var _g11 = s2.y2 + 1;
					while(_g21 < _g11) {
						var obj1 = this.cursor.s.lines[_g21++];
						var out = { };
						var _g41 = s2.x1;
						var _g31 = s2.x2 + 1;
						while(_g41 < _g31) {
							var c1 = this.cursor.s.columns[_g41++];
					...
					var _g32 = s2.x1;
					var _g22 = s2.x2 + 1;
					while(_g32 < _g22) _g12.push(this.cursor.s.columns[_g32++]);
		...
		case 115:
			if(this.cursor.s != null && this.cursor.x >= 0) {
				...
				case 6:
					var s5 = _g14[2];
					var _this = this.smap;
					var sd = __map_reserved[s5] != null?_this.getReserved(s5):_this.h[s5];
					if(sd != null) {
						var _this1 = sd.index;
						...
						if(k != null) {
							...
							if(index >= 0) {
								...
								if(__map_reserved[s5] != null) {
									_this2.setReserved(s5,value);
								} else {
									_this2.h[s5] = value;
								}
	...
	,changed: function(sheet,c,index,old) {
		...
		case 14:
			...
			if(newV != null && oldV != null && oldV.file != newV.file && !sys_FileSystem.exists(this.getAbsPath(oldV.file)) && sys_FileSystem.exists(this.getAbsPath(newV.file))) {
				...
				while(_g2 < _g1) {
					var t = Reflect.field(sheet.lines[_g2++],c.name);
	...
	,valueHtml: function(c,v,sheet,obj) {
		...
		case 17:
			...
			var ps1 = (__map_reserved[key3] != null?_this4.getReserved(key3):_this4.h[key3]).s;
			var out1 = [];
			var _g12 = 0;
			var _g21 = ps1.columns;
			while(_g12 < _g21.length) {
				var c2 = _g21[_g12];
				++_g12;
				...
				out1.push(c2.name + " : " + this.valueHtml(c2,pval,ps1,v));
			}
			return out1.join("<br/>");
		case 9:
			var name = _g[2];
			...
			if(cas.args.length > 0) {
				...
				while(_g22 < _g13) {
					var i1 = _g22++;
					out2.push(this.valueHtml(cas.args[i1 - 1],a1[i1],sheet,this));
				}
				str += out2.join(",");
				str += ")";
			}
			...
			i.blur(function(_) {
				...
				if(nv == "" && c.opt) {
					...
					case 9:
						...
						try {
							var _this2 = _g.tmap;
							val2 = _g.parseTypeVal(__map_reserved[t1] != null?_this2.getReserved(t1):_this2.h[t1],nv);
						} catch( e2 ) {
							val2 = null;
						}
						break;
	...
	,fillProps: function(content,sheet,props) {
		...
		while(_g11 < _g21.length) {
			...
			l[0].click((function(index1) {
				return function(e) {
					_g3.setCursor(sheet,0,index1[0]);
					e.stopPropagation();
				};
			})([tmp]));
			th.mousedown((function(l1,c1) {
				return function(e1) {
					if(e1.which == 3) {
						var f = $bind(_g3,_g3.popupColumn);
						var a1 = sheet;
						var c2 = c1[0];
						haxe_Timer.delay((function() {
							return function() {
			...
			td[0].dblclick((function(td1,c3) {
				return function(e2) {
					_g3.editCell(c3[0],td1[0],sheet,0);
					e2.preventDefault();
					e2.stopPropagation();
				};
			})(td,c));
		...
		sel.change(function(e3) {
			...
			while(_g13 < available.length) {
				...
				++_g13;
				if(c5.name == v1) {
					props[c5.name] = _g3.getDefault(c5,true);
					_g3.save();
					_g3.refresh();
					return;
				}
		...
		while(_g31 < _g22) {
			...
			while(_g5 < _g41) {
				...
				v[0].click((function(index1,cindex1) {
					return function(e4) {
						if(!inTodo) {
							if(e4.shiftKey && _g4.cursor.s == sheet) {
								_g4.cursor.select = { x : cindex1[0], y : index1[0]};
								_g4.updateCursor();
								e4.stopImmediatePropagation();
							} else {
								_g4.setCursor(sheet,cindex1[0],index1[0]);
							}
						}
						e4.stopPropagation();
					};
				...
				var set = [(function(html1,v1,val1,obj1,index2,c4) {
					return function(val2) {
						...
						v1[0].html(html1[0]);
						_g4.changed(sheet,c4[0],index2[0],old);
					};
				...
				case 17:
					...
					v[0].click((function(key5,html4,l3,v6,val5,obj5,index5,c9,cindex3) {
						return function(e9) {
							var next1 = l3[0].next("tr.list");
							if(next1.length > 0) {
								if(next1.data("name") == c9[0].name) {
									next1.change();
									return;
								}
								next1.change();
							}
							next1 = $("<tr>").addClass("list").data("name",c9[0].name);
							$("<td>").appendTo(next1);
							...
							var _this3 = SheetData.model.smap;
							var key6 = sheet.name + "@" + c9[0].name;
							var psheet1 = (__map_reserved[key6] != null?_this3.getReserved(key6):_this3.h[key6]).s;
							if(val5[0] == null) {
								val5[0] = { };
								obj5[0][c9[0].name] = val5[0];
							}
							psheet1 = { columns : psheet1.columns, props : psheet1.props, name : psheet1.name, path : key5[0], parent : { sheet : sheet, column : cindex3[0], line : index5[0]}, lines : [val5[0]], separators : []};
							_g4.fillProps(content2,psheet1,val5[0]);
							next1.insertAfter(l3[0]);
							v6[0].text("...");
							var _this4 = _g4.openedList;
							if(__map_reserved[key5[0]] != null) {
								_this4.setReserved(key5[0],true);
							} else {
								_this4.h[key5[0]] = true;
							}
							next1.change((function(key7,html5,v7,val6,obj6,c10) {
								return function(e10) {
									if(c10[0].opt && Reflect.fields(val6[0]).length == 0) {
										val6[0] = null;
										Reflect.deleteField(obj6[0],c10[0].name);
									}
									html5[0] = _g4.valueHtml(c10[0],val6[0],sheet,obj6[0]);
									v7[0].html(html5[0]);
									...
									_g4.openedList.remove(key7[0]);
									e10.stopPropagation();
								};
							})(key5,html4,v6,val5,obj5,c9));
							if(inTodo) {
								if(_g4.cursor.s != null && SheetData.getPath(_g4.cursor.s) == SheetData.getPath(psheet1)) {
									_g4.cursor.s = psheet1;
									_g4.checkCursor = false;
								}
							} else {
								div1.slideDown(100);
								_g4.setCursor(psheet1);
							}
							e9.stopPropagation();
						};
				...
				case 12:
					break;
				case 13:
					v[0].find("input").addClass("deletable").change((function(obj7,c11) {
						return function(e11) {
							if(Reflect.field(obj7[0],c11[0].name) != null) {
								Reflect.deleteField(obj7[0],c11[0].name);
								_g4.refresh();
								_g4.save();
							}
						};
					...
					break;
				case 14:
					v[0].find("div").addClass("deletable").change((function(obj8,c12) {
						return function(e12) {
							if(Reflect.field(obj8[0],c12[0].name) != null) {
								Reflect.deleteField(obj8[0],c12[0].name);
								_g4.refresh();
								_g4.save();
							}
						};
					})(obj,c));
					v[0].dblclick((function(set3,v9,val7,index6,c13) {
						return function(_4) {
							var rv = val7[0];
							...
							if(file == null) {
								var i3 = index6[0] - 1;
								while(i3 >= 0) {
									var v21 = Reflect.field(sheet.lines[i3--],c13[0].name);
									if(v21 != null) {
										file = v21.file;
										size = v21.size;
										break;
							...
							dialog.find(".tileView").css({ backgroundImage : "url(\"" + _g4.getAbsPath(file) + "\")"}).mousemove((function() {
								return function(e13) {
									var off = $(this).offset();
									posX = (e13.pageX - off.left) / size | 0;
									posY = (e13.pageY - off.top) / size | 0;
				...
				default:
					v[0].dblclick((function(v14,index7,c14) {
						return function(e16) {
							_g4.editCell(c14[0],v14[0],sheet,index7[0]);
						};
					})(v,index,c));
				}
			}
			...
			while(_g32 < _g23) {
				...
				c15.click((function(l6,index9) {
					return function(_13) {
						...
						if(found == null) {
							found = new Level(_g4,sheet,index9[0]);
	...
	,editTypes: function() {
		...
		apply.click(function(_2) {
			...
			while(_g15 < tpairs.length) {
				...
				++_g15;
				if(p2.b == null) {
					HxOverrides.remove(_g.data.customTypes,p2.a);
				} else {
					try {
	...
	,newColumn: function(sheetName,ref,index) {
		...
		if(ref != null) {
			...
			case 6:
				var sname = _g3[2];
				form.find("[name=sheet]").val("" + Lambda.indexOf(this.data.sheets,Lambda.find(this.data.sheets,function(s11) {
					return s11.name == sname;
				})));
				break;
			case 12:
				var sname1 = _g3[2];
				form.find("[name=sheet]").val("" + Lambda.indexOf(this.data.sheets,Lambda.find(this.data.sheets,function(s11) {
					return s11.name == sname1;
				})));
				break;
			case 9:
	...
	,createColumn: function() {
		...
		$("#newcol").hide();
		var _g_i1 = 0;
		while(_g_i1 < cols.length) $(cols[_g_i1++]).val("");
		this.refresh();
		this.save();
	}
	,initContent: function() {
				...
				return function(_1) {
					...
					$("<input>").val(s2[0].name).appendTo(li1[0]).focus().blur((function(li2,s3) {
						return function(_2) {
							...
							if(f != null) {
								if(f.s != s3[0]) {
									_g2.error("Sheet name already in use");
								}
								return;
							}
							var old = s3[0].name;
	...
	,initMenu: function() {
		...
		this.window.on("close",function() {
			...
			if(!_g.prefs.windowPos.max) {
				_g.prefs.windowPos = { x : _g.window.x, y : _g.window.y, w : _g.window.width, h : _g.window.height, max : false};
			}
			_g.savePrefs();
		...
		if(sheet.props.level != null && (c.name == "width" || c.name == "height")) {
			...
			while(_g11 < _g21.length) {
				...
				case 12:
					...
					var _g44 = [];
					var _g5 = 0;
					while(_g5 < 256) _g44.push(_g5++);
					var odat1 = cdb__$Types_Layer_$Impl_$.decode(v2,_g44);
					var ndat1 = [];
					var _g6 = 0;
					var _g51 = newH;
					while(_g6 < _g51) {
...
haxe_Serializer.prototype = {
	...
	,serialize: function(v) {
		...
		case 6:
			...
			case haxe_io_Bytes:
				...
				if(b64 == null) {
					...
					var _g21 = 0;
					var _g11 = haxe_Serializer.BASE64.length;
					while(_g21 < _g11) {
						var i2 = _g21++;
		...
		case 7:
			...
			var _g22 = 2;
			var _g12 = l1;
			while(_g22 < _g12) this.serialize(v[_g22++]);
			if(this.useCache) {
				this.cache.push(v);
			}
			break;
...
cdb_BinSerializer.prototype = {
	...
	,serializeData: function(d,v) {
		...
		case 5:
			...
			var _this2 = this.out;
			var src2 = haxe_io_Bytes.ofString(str);
			var b22 = src2.b;
			var _g12 = 0;
			var _g5 = src2.length;
	...
	while(_g7 < allBorders.length) {
		...
		if(gid1 != null) {
			...
			if(_g14 == null) {
				var _g22 = [];
				var _g41 = tid1.id + 1;
				while(_g41 < maxGid) _g22.push(_g41++);
				gids = _g22;
			} else {
				switch(_g14) {
				case "lower":
					...
					var _g42 = 0;
					var _g32 = tid1.id;
					while(_g42 < _g32) _g23.push(_g42++);
					gids = _g23;
					break;
				default:
					continue;
		...
		if(tid1 != null) {
			...
			if(_g15 == null) {
				...
				var _g43 = 0;
				var _g33 = gid1.id;
				while(_g43 < _g33) _g24.push(_g43++);
				tids = _g24;
			} else {
				switch(_g15) {
				case "upper":
					var _g25 = [];
					var _g44 = gid1.id + 1;
					while(_g44 < maxGid) _g25.push(_g44++);
					tids = _g25;
					break;
				default:
					continue;
				...
				while(_g5 < _g4) {
					...
					if((bits & 26) == 26) {
						...
						if(a6.length != 0) {
							...
							out.push(x);
							out.push(y);
							out.push(a6.length == 1?a6[0]:a6[_g6.random(x + y * width) % a6.length]);
						}
					}
					if((bits & 74) == 74) {
						var a7 = bb[14];
						if(a7.length != 0) {
							...
							out.push(x);
							out.push(y);
							out.push(a7.length == 1?a7[0]:a7[_g6.random(x + y * width) % a7.length]);
						}
					}
					if((bits & 82) == 82) {
						var a8 = bb[15];
					...
					if((bits & 16) == 16) {
						...
						if(a17.length != 0) {
							...
							out.push(x);
							out.push(y);
							out.push(a17.length == 1?a17[0]:a17[_g6.random(x + y * width) % a17.length]);
						}
					}
					if((bits & 64) == 64) {
						var a18 = bb[1];
					...
					if(f1) {
						...
						if(a20.length != 0) {
							...
							out.push(x);
							out.push(y1);
							out.push(a20.length == 1?a20[0]:a20[_g6.random(x + y1 * width) % a20.length]);
						}
					}
					var f2 = false;
					if((bits & 4) == 4) {
					...
					if(f2) {
						...
						if(a22.length != 0) {
							...
							out.push(x);
							out.push(y2);
							out.push(a22.length == 1?a22[0]:a22[_g6.random(x + y2 * width) % a22.length]);
						}
					}
					if((bits & 32) == 32) {
						var a23 = bb[2];
					...
					if((bits & 128) == 128) {
						...
						if(a24.length != 0) {
							out.push(x);
							out.push(y);
							out.push(a24.length == 1?a24[0]:a24[_g6.random(x + y * width) % a24.length]);
						}
					}
				}
			}
...
haxe_Unserializer.prototype = {
	...
	,unserialize: function() {
		...
		case 106:
			...
			var tag = edecl1.__constructs__.slice()[index];
			if(tag == null) {
		...
		case 77:
			...
			this.cache.push(h2);
			var buf4 = this.buf;
			while(this.buf.charCodeAt(this.pos) != 104) h2.set(this.unserialize(),this.unserialize());
			this.pos++;
			return h2;
		case 118:
			var d;
...
lvl_Image3D.prototype = $extend(lvl_Image.prototype,{
	...
	,drawMat: function(i,m) {
		...
		var tmp = (i.originX + 0.001) / _g.curTexture.width;
		this.curDraw[this.drawPos++] = tmp;
		var tmp1 = i.originY / _g.curTexture.height;
		...
		var tmp2 = (i.originX + i.width) / _g.curTexture.width;
		this.curDraw[this.drawPos++] = tmp2;
		var tmp3 = i.originY / _g.curTexture.height;
		...
		var tmp4 = (i.originX + 0.001) / _g.curTexture.width;
		this.curDraw[this.drawPos++] = tmp4;
		var tmp5 = (i.originY + i.height + -0.01) / _g.curTexture.height;
		...
		var tmp6 = (i.originX + i.width) / _g.curTexture.width;
		this.curDraw[this.drawPos++] = tmp6;
		var tmp7 = (i.originY + i.height + -0.01) / _g.curTexture.height;
...
lvl_LayerData.prototype = $extend(lvl_LayerGfx.prototype,{
	...
	,getData: function() {
		...
		case 2:
			...
			while(_g21 < _g11) b1.writeUInt16(data1[_g21++]);
			if(t.file == null) {
				return null;
			} else {
			...
			case "objects":
				...
				case 2:
					...
					while(_g22 < objs.length) {
						var o3 = objs[_g22];
						++_g22;
						_g11.push({ x : o3.x, y : o3.y, o : o3.id, flip : false, rot : 0});
					}
					this.data = lvl_LayerInnerData.TileInstances(_g[2],_g11);
					this.dirty = true;
					break;
				default:
							...
							while(_g41 < _g32) {
								...
								while(_g62 < _g52) {
									...
									if(x2 < width && y2 < height) {
										_g12[x2 + y2 * width] = i.o + dx2 + dy2 * this.stride + 1;
									}
								}
							}
						}
					}
					this.data = lvl_LayerInnerData.Tiles(_g3[2],_g12);
					this.dirty = true;
					break;
				default:
	...
	,draw: function(view) {
		...
		case 3:
			...
			var _g1 = 0;
			while(_g1 < insts.length) {
				var i1 = insts[_g1];
				++_g1;
				var x3 = i1.x * size | 0;
				var y3 = i1.y * size | 0;
				var obj = objs.h[i1.o];
				var w = (obj == null?1:obj.w) * size;
				var h = (obj == null?1:obj.h) * size;
				var rot = i1.rot;
				var flip = i1.flip;
				mat.a = 1;
				mat.b = 0;
				...
				mat.x += Math.abs(mat.a * w * 0.5 + mat.c * h * 0.5);
				mat.y += Math.abs(mat.b * w * 0.5 + mat.d * h * 0.5);
				mat.x += x3;
				mat.y += y3;
				if(obj == null) {
		...
		case 1:
			...
			if(idCol == null) {
				...
				while(_g12 < objs1.length) {
					...
					var px1 = o1.x * size | 0;
					var py1 = o1.y * size | 0;
					view.fillRect(px1,py1,w1,1,col1);
					view.fillRect(px1,py1 + h1 - 1,w1,1,col1);
					view.fillRect(px1,py1 + 1,1,h1 - 2,col1);
					view.fillRect(px1 + w1 - 1,py1 + 1,1,h1 - 2,col1);
				}
			} else {
				var _g13 = 0;
				while(_g13 < objs1.length) {
					...
					var _this = this.idToIndex;
					var k2 = __map_reserved[id] != null?_this.getReserved(id):_this.h[id];
					if(k2 == null) {
					...
					view.fillRect(o2.x * size | 0,o2.y * size | 0,(this.hasSize?o2.width * size:size) | 0,(this.hasSize?o2.height * size:size) | 0,this.colors[k2] | -16777216);
				}
			}
			break;
		}
	...
	,layerChanged: function(l) {
		...
		jsel.mousedown(function(e2) {
			...
			if(e2.shiftKey) {
				...
				start_down = true;
				_g.level.set_mouseCapture(jsel);
			...
			if(prop != null) {
				...
				case 6:
					...
					if(pick) {
						var key1 = Reflect.field(_g.getTileProp(x1,y1),prop.name);
						var _this1 = c.idToIndex;
						var idx = __map_reserved[key1] != null?_this1.getReserved(key1):_this1.h[key1];
						_g.modeCursor = idx == null?-1:idx;
						_g.level.setCursor();
						return;
					}
					var v1 = _g.modeCursor < 0?_g.getDefault(prop):c.indexToId[_g.modeCursor];
					if(v1 == null) {
						Reflect.deleteField(_g.getTileProp(x1,y1),prop.name);
					} else {
						_g.getTileProp(x1,y1)[prop.name] = v1;
					}
					_g.saveTileProps();
					break;
				case 5:
					if(pick) {
						...
						_g.modeCursor = idx1 == null?-1:idx1;
						_g.level.setCursor();
						return;
					}
					var v2 = _g.modeCursor < 0?_g.getDefault(prop):_g.modeCursor;
					if(v2 == null) {
						Reflect.deleteField(_g.getTileProp(x1,y1),prop.name);
					} else {
						_g.getTileProp(x1,y1)[prop.name] = v2;
					}
					_g.saveTileProps();
					break;
				default:
		...
		jsel.mousemove(function(e3) {
			...
			if(l.tileProps != null) {
				_g.level.content.find(".cursorPosition").text(infos);
			} else {
				_g.p.find(".infos").text(infos);
			}
			if(!start_down) {
				return;
			}
		...
		jsel.mouseleave(function(e4) {
			if(l.tileProps != null) {
				_g.level.content.find(".cursorPosition").text("");
			} else {
				_g.p.find(".infos").text("");
			}
			curPreview = -1;
			jpreview.hide();
		});
		...
		this.p.mousemove(function(e5) {
			...
			if(_g.level.selection == null) {
				_g.level.cursor.hide();
			}
		});
		...
		case 3:
			...
			while(_g13 < insts.length) {
				...
				while(_g31 < _g2) {
					...
					var _g5 = 0;
					var _g4 = t.w;
					while(_g5 < _g4) used[i.o + _g5++ + dy * l.stride] = true;
				}
			}
			break;
		}
			...
			case 6:
				...
				while(_g22 < _g16) {
					...
					while(_g42 < _g33) {
						...
						var v1 = __map_reserved[r] != null?_this2.getReserved(r):_this2.h[r];
						if(v1 == null || r == def) {
							continue;
						}
	...
	,option: function(name,val) {
		...
		case "value":
			...
			if(s2 != null) {
				...
				if(v1 == null) {
					Reflect.deleteField(s2.opts,"value");
				} else {
					s2.opts.value = v1;
				}
		...
		case "priority":
			...
			if(s3 != null) {
				s3.opts.priority = Std.parseInt(val);
			}
			break;
		case "border_in":
			...
			if(s4 != null) {
				if(val == "null") {
					Reflect.deleteField(s4.opts,"borderIn");
				} else {
					s4.opts.borderIn = val;
				}
			}
			break;
		case "border_out":
			...
			if(s5 != null) {
				if(val == "null") {
					Reflect.deleteField(s5.opts,"borderOut");
				} else {
					s5.opts.borderOut = val;
				}
			}
			break;
		case "border_mode":
			...
			if(s6 != null) {
				if(val == "null") {
					Reflect.deleteField(s6.opts,"borderMode");
				} else {
					s6.opts.borderMode = val;
				}
			}
			break;
		case "small":
