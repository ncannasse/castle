		...
		while(_g2 < _g11.length) {
			...
			case 8:
				...
				if(!(sheet.hasColumn("x",[cdb_ColumnType.TInt]) && sheet.hasColumn("y",[cdb_ColumnType.TInt]))) {
					if(sheet.hasColumn("x",[cdb_ColumnType.TFloat])) {
						floatCoord = sheet.hasColumn("y",[cdb_ColumnType.TFloat]);
					} else {
						floatCoord = false;
					}
					tmp = floatCoord;
	...
	,editProps: function(l,index) {
		...
		while(_g < _g1.length) {
			...
			var tmp = (function(td3,c2) {
				return function(e3) {
					var psheet = new data_Sheet(null,{ columns : l.baseSheet.sheet.columns, props : l.baseSheet.sheet.props, name : l.baseSheet.sheet.name, lines : Reflect.field(_gthis.obj,l.name), separators : []},l.baseSheet.getPath() + ":" + index,{ sheet : _gthis.sheet, column : Lambda.indexOf(_gthis.sheet.sheet.columns,Lambda.find(_gthis.sheet.sheet.columns,(function() {
						return function(c3) {
...
Main.prototype = $extend(Model.prototype,{
				...
				if(_g7[1] == 6) {
					...
					if(sd != null) {
						...
						if(k != null) {
							...
							if(index >= 0) {
								this.sheetCursors.set(s6,{ s : sd, x : 0, y : index});
								this.selectSheet(sd);
							}
						}
					}
		...
		while(_g2 < results.length) {
			...
			l.click((function(slast1,rs1) {
				return function(e) {
					...
					var value = { s : new data_Sheet(_gthis.base,{ name : slast1[0].s.sheet.name, separators : [], lines : [], columns : [], props : { }},key), x : -1, y : rs1[0].o.indexes[rs1[0].o.indexes.length - 1]};
					var _this1 = this1;
					if(__map_reserved[key1] != null) {
	...
	,valueHtml: function(c,v,sheet,obj) {
		...
		case 0:
			...
			} else if(this.base.getSheet(sheet.sheet.name).index.get(v).obj == obj) {
				return v;
			} else {
	...
	,popupLine: function(sheet,index) {
		...
		nsep.click = function() {
			if(sepIndex >= 0) {
				...
				var _g11 = 0;
				var _g2 = sheet.sheet.separators.length;
				while(_g11 < _g2) {
					var i = _g11++;
	...
	,popupSheet: function(s,li) {
		...
		nright.click = function() {
			var found = null;
			var _g12 = 0;
			var _g3 = _gthis.base.sheets.length;
			while(_g12 < _g3) {
				var i1 = _g12++;
	...
	,editCell: function(c,v,sheet,index) {
		...
		case 0:case 1:case 3:case 4:case 9:case 16:
			...
			if(val != null) {
				...
				case 9:
					var t = _g1[2];
					var tmp1 = this.base.typeValToString(this.base.getCustomType(t),val);
					i.val(tmp1);
					break;
				case 16:
					var tmp2 = JSON.stringify(val);
					i.val(tmp2);
					break;
				default:
			...
			i.blur(function(_) {
				var nv = i.val();
				var old1 = val;
				var prevObj = c.type == cdb_ColumnType.TId && old1 != null ? _gthis.base.getSheet(sheet.sheet.name).index.get(val) : null;
				var prevTarget = null;
				if(nv == "" && c.opt) {
					if(val != null) {
						html = null;
					...
					switch(_g3[1]) {
					case 0:
						if(_gthis.base.r_ident.match(nv)) {
							val2 = nv;
						} else {
							val2 = null;
						}
					...
					if(val2 != val && val2 != null) {
						prevTarget = _gthis.base.getSheet(sheet.sheet.name).index.get(val2);
				...
				editDone();
				if(c.type == cdb_ColumnType.TId && prevObj != null && old1 != val && (prevObj.obj == obj && _gthis.base.getSheet(sheet.sheet.name).index.get(old1) != null || prevTarget != null && _gthis.base.getSheet(sheet.sheet.name).index.get(val).obj != prevTarget.obj)) {
					_gthis.refresh();
					return;
				}
			});
		...
		case 5:
			...
			while(_g11 < _g5) {
				var i1 = _g11++;
				var tmp3 = $("<option>");
				var tmp4 = val == i1 ? "selected" : "_sel";
				tmp3.attr("value","" + i1).attr(tmp4,"selected").text(values[i1]).appendTo(s);
			}
			if(c.opt) {
		...
		case 7:
			var i3 = $("<input>").attr("type","file").css("display","none").change(function(e8) {
				...
				val = md5;
				obj[c.name] = val;
				var i4 = _gthis.valueHtml(c,val,sheet,obj);
				v.html(i4);
				_gthis.changed(sheet,c,index,old);
				j.remove();
			});
		...
		case 10:
			...
			while(_g13 < _g8) {
				var i5 = [_g13++];
				var f1 = $("<input>").attr("type","checkbox").prop("checked",(val & 1 << i5[0]) != 0).change((function(i6) {
					return function(e11) {
						val &= ~(1 << i6[0]);
				...
				$("<label>").text(values1[i5[0]]).appendTo(div).append(f1);
			}
			v.empty();
			v.append(div);
		...
		case 11:
			...
			spect.spectrum({ color : "#" + StringTools.hex(val,6), showInput : true, showButtons : false, change : function() {
				...
				val = color;
				obj[c.name] = color;
				var tmp5 = _gthis.valueHtml(c,val,sheet,obj);
				v.html(tmp5);
				_gthis.save();
			}}).spectrum("show");
			break;
		...
		while(_g31 < _g22) {
			...
			while(_g5 < _g4) {
				...
				case 8:
					...
					var tmp3 = (function(key1,html2,l2,v3,val3,obj3,index5,c7,cindex2) {
						return function(e7) {
							...
							psheet = new data_Sheet(_gthis.base,{ columns : psheet.sheet.columns, props : psheet.sheet.props, name : psheet.sheet.name, lines : val3[0], separators : []},key1[0],{ sheet : sheet, column : cindex2[0], line : index5[0]});
				...
				case 17:
					...
					var tmp23 = (function(key4,html4,l3,v11,val6,obj7,index7,c12,cindex3) {
						return function(e14) {
							...
							psheet1 = new data_Sheet(_gthis1,{ columns : psheet2, props : psheet3, name : psheet4, lines : _g61, separators : []},key4[0],{ sheet : sheet, column : cindex3[0], line : index7[0]});
					...
					break;
				default:
					var tmp27 = (function(v14,index8,c14) {
						return function(e16) {
...
var data_Database = function() {
	this.r_ident = new EReg("^[A-Za-z_][A-Za-z0-9_]*$","");
	this.data = { sheets : [], customTypes : [], compress : false};
	this.sheets = [];
	this.sync();
};
$hxClasses["data.Database"] = data_Database;
data_Database.__name__ = ["data","Database"];
data_Database.prototype = {
	...
	,addSheet: function(s) {
		var sobj = new data_Sheet(this,s);
	...
	,parseVal: function(t,val) {
		...
		case 6:
			var t1 = t[2];
			var r = this.getSheet(t1).index.get(val);
			if(r == null) {
	...
	,__class__: data_Database
};
var data_Sheet = function(base,sheet,path,parent) {
	this.base = base;
	this.sheet = sheet;
	this.path = path;
	this.parent = parent;
};
$hxClasses["data.Sheet"] = data_Sheet;
data_Sheet.__name__ = ["data","Sheet"];
data_Sheet.prototype = {
	get_lines: function() {
				...
				if(this.sheet.props.displayIcon == c.name) {
					...
					while(_g8 < _g13.length) {
						...
						if(cid1.type == cdb_ColumnType.TId) {
							...
							if(id1 != null) {
								this.index.get(id1).ico = Reflect.field(obj2,c.name);
							}
						}
					}
				}
			...
			if(this.sheet.props.displayIcon == c.name) {
				...
				while(_g10 < _g15.length) {
					...
					if(cid3.type == cdb_ColumnType.TId) {
						...
						if(id3 != null) {
							this.index.get(id3).ico = Reflect.field(obj4,c.name);
						}
					}
				}
			}
...
var hxbit_PropTypeDesc = $hxClasses["hxbit.PropTypeDesc"] = { __ename__ : ["hxbit","PropTypeDesc"], __constructs__ : ["PInt","PFloat","PBool","PString","PBytes","PSerializable","PEnum","PMap","PArray","PObj","PAlias","PVector","PNull","PUnknown","PDynamic","PInt64","PFlags"] };
