...
Main.prototype = $extend(Model.prototype,{
	...
	,fillTable: function(content,sheet) {
			...
			while(_g5 < _g44) {
				...
				case 17:
					...
					v[0].click((function(key11,html11,l11,v11,val11,obj11,index11,c11,cindex11) {
						return function(e11) {
							var next11 = l11[0].next("tr.props");
							if(next11.length > 0) {
							...
							_g4.fillTable(content11,psheet12);
				...
				case 14:
					...
					v[0].dblclick((function(set16,v16,val16,index16,c16) {
						return function(_16) {
							...
							if(file == null) {
								_g4.chooseFile((function(v21) {
									return function(path21) {
										file = path21;
										setVal();
										v21[0].dblclick();
									};
							...
							var tmp17 = $(".tileSelect").parent().html();
							var dialog = $(tmp17).prependTo($("body"));
							var maxWidth = 1000000;
							var maxHeight = 1000000;
							var tmp18 = "url(\"" + _g4.getAbsPath(file) + "\")";
							var tmp19 = (function() {
								return function(e18) {
									var off = $(this).offset();
							...
							dialog.find(".tileView").css({ backgroundImage : tmp18}).mousemove(tmp19).click((function() {
								return function(_21) {
									setVal();
									dialog.remove();
									_g4.save();
							...
							dialog.find("[name=file]").click((function(v25) {
								return function(_26) {
									_g4.chooseFile((function(v26) {
										return function(f26) {
											file = f26;
											dialog.remove();
											setVal();
							...
							dialog.keydown(tmp20).keypress((function() {
								return function(e26) {
									e26.stopPropagation();
								};
							})());
							dialog.show();
							var i20 = window.document.createElement("img");
							i20.onload = (function() {
								return function(_27) {
									maxWidth = i20.width;
									maxHeight = i20.height;
									dialog.find(".tileView").height(i20.height).width(i20.width);
									dialog.find(".tilePath").text(file + " (" + i20.width + "x" + i20.height + ")");
								};
							})();
							i20.src = _g4.getAbsPath(file);
						};
					})(set,v,val,index,c));
					break;
				default:
					v[0].dblclick((function(v27,index27,c27) {
						return function(e27) {
							_g4.editCell(c27[0],v27[0],sheet,index27[0]);
						};
					})(v,index,c));
				}
			}
		...
		if(sheet.lines.length == 0) {
			var l27 = $("<tr><td colspan=\"" + (sheet.columns.length + 1) + "\"><a href=\"javascript:_.insertLine()\">Insert Line</a></td></tr>");
			l27.find("a").click(function(_28) {
				_g4.setCursor(sheet);
			});
			lines.push(l27);
		}
		if(sheet.props.level != null) {
			var col28 = $("<td style='width:35px'>");
			cols.prepend(col28);
			var _g328 = 0;
			var _g228 = sheet.lines.length;
			while(_g328 < _g228) {
				var index28 = [_g328++];
				var l28 = [lines[index28[0]]];
				var c28 = $("<input type='submit' value='Edit'>");
				$("<td>").append(c28).prependTo(l28[0]);
				c28.click((function(l29,index29) {
					return function(_29) {
						l29[0].click();
						var found = null;
						var _g529 = 0;
						var _g629 = _g4.levels;
						while(_g529 < _g629.length) {
							var l30 = _g629[_g529];
							++_g529;
							if(l30.sheet == sheet && l30.index == index29[0]) found = l30;
						}
						if(found == null) {
							found = new Level(_g4,sheet,index29[0]);
		...
		var snext = 0;
		var _g330 = 0;
		var _g230 = lines.length;
		while(_g330 < _g230) {
			var i30 = _g330++;
			while(sheet.separators[snext] == i30) {
				var sep = $("<tr>").addClass("separator").append("<td colspan=\"" + (colCount + 1) + "\">").appendTo(content);
				var content30 = [sep.find("td")];
				var title = [sheet.props.separatorTitles != null?sheet.props.separatorTitles[snext]:null];
				if(title[0] != null) content30[0].text(title[0]);
				var pos = [snext];
				sep.dblclick((function(pos30,title30,content31) {
					return function(e31) {
						content31[0].empty();
						var tmp31 = content31[0];
						var tmp32 = title30[0] == null?"":title30[0];
						var tmp33 = (function(pos32,title32,content32) {
							return function(_32) {
								title32[0] = $(this).val();
								$(this).remove();
								content32[0].text(title32[0]);
								var titles = sheet.props.separatorTitles;
								if(titles == null) titles = [];
								while(titles.length < pos32[0]) titles.push(null);
								titles[pos32[0]] = title32[0] == ""?null:title32[0];
						...
						})(pos30,title30,content31);
						var tmp34 = (function() {
							return function(e33) {
								e33.stopPropagation();
							};
						})();
						$("<input>").appendTo(tmp31).focus().val(tmp32).blur(tmp33).keypress(tmp34).keydown((function(title34,content34) {
							return function(e34) {
								if(e34.keyCode == 13) {
									$(this).blur();
									e34.preventDefault();
								} else if(e34.keyCode == 27) content34[0].text(title34[0]);
								e34.stopPropagation();
							};
						})(title30,content31));
					};
			...
			content.append(lines[i30]);
		}
		inTodo = true;
		var _g234 = 0;
		while(_g234 < todo.length) {
			var t34 = todo[_g234];
			++_g234;
			t34();
		}
	...
	,initContent: function() {
		...
		while(_g7 < old.length) {
			...
			if(!(function($this) {
				...
				$r = __map_reserved[key] != null?_this8.existsReserved(key):_this8.h.hasOwnProperty(key);
				return $r;
			}(this))) continue;
			var s8 = (function($this) {
				var $r;
				var name7 = level.sheetPath;
				var tmp;
				var _this7 = $this.smap;
				if(__map_reserved[name7] != null) tmp = _this7.getReserved(name7); else tmp = _this7.h[name7];
				$r = tmp.s;
				return $r;
			}(this));
...
cdb__$BinSerializer_SchemaKind.SEnum = function(e) { var $x = ["SEnum",0,e]; $x.__enum__ = cdb__$BinSerializer_SchemaKind; return $x; };
cdb__$BinSerializer_SchemaKind.SAnon = function(e) { var $x = ["SAnon",1,e]; $x.__enum__ = cdb__$BinSerializer_SchemaKind; return $x; };
cdb__$BinSerializer_SchemaKind.SMulti = function(a) { var $x = ["SMulti",2,a]; $x.__enum__ = cdb__$BinSerializer_SchemaKind; return $x; };
...
cdb__$BinSerializer_SData.DNull = function(d) { var $x = ["DNull",6,d]; $x.__enum__ = cdb__$BinSerializer_SData; return $x; };
cdb__$BinSerializer_SData.DArray = function(d) { var $x = ["DArray",7,d]; $x.__enum__ = cdb__$BinSerializer_SData; return $x; };
cdb__$BinSerializer_SData.DSchema = function(id) { var $x = ["DSchema",8,id]; $x.__enum__ = cdb__$BinSerializer_SData; return $x; };
cdb__$BinSerializer_SData.DAnon = function(a) { var $x = ["DAnon",9,a]; $x.__enum__ = cdb__$BinSerializer_SData; return $x; };
cdb__$BinSerializer_SData.DIntMap = function(d) { var $x = ["DIntMap",10,d]; $x.__enum__ = cdb__$BinSerializer_SData; return $x; };
cdb__$BinSerializer_SData.DStringMap = function(d) { var $x = ["DStringMap",11,d]; $x.__enum__ = cdb__$BinSerializer_SData; return $x; };
cdb__$BinSerializer_SData.DEnumMap = function(e,d) { var $x = ["DEnumMap",12,e,d]; $x.__enum__ = cdb__$BinSerializer_SData; return $x; };
...
cdb_jq_Message.Create = function(id,name,attr) { var $x = ["Create",0,id,name,attr]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.AddClass = function(id,name) { var $x = ["AddClass",1,id,name]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.RemoveClass = function(id,name) { var $x = ["RemoveClass",2,id,name]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.Append = function(id,to) { var $x = ["Append",3,id,to]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.InsertAt = function(id,to,pos) { var $x = ["InsertAt",4,id,to,pos]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.CreateText = function(id,text,pid) { var $x = ["CreateText",5,id,text,pid]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.Reset = function(id) { var $x = ["Reset",6,id]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.Dock = function(pid,id,dir,size) { var $x = ["Dock",7,pid,id,dir,size]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.Remove = function(id) { var $x = ["Remove",8,id]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.Event = function(id,name,eid) { var $x = ["Event",9,id,name,eid]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.SetAttr = function(id,att,val) { var $x = ["SetAttr",10,id,att,val]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.SetStyle = function(id,st,val) { var $x = ["SetStyle",11,id,st,val]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.Trigger = function(id,name) { var $x = ["Trigger",12,id,name]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.Special = function(id,name,args,eid) { var $x = ["Special",13,id,name,args,eid]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.Anim = function(id,name,dur) { var $x = ["Anim",14,id,name,dur]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.Dispose = function(id,events) { var $x = ["Dispose",15,id,events]; $x.__enum__ = cdb_jq_Message; return $x; };
cdb_jq_Message.Unbind = function(events) { var $x = ["Unbind",16,events]; $x.__enum__ = cdb_jq_Message; return $x; };
var cdb_jq_Answer = $hxClasses["cdb.jq.Answer"] = { __ename__ : ["cdb","jq","Answer"], __constructs__ : ["Event","SetValue","Done"] };
cdb_jq_Answer.Event = function(eid,props) { var $x = ["Event",0,eid,props]; $x.__enum__ = cdb_jq_Answer; return $x; };
cdb_jq_Answer.SetValue = function(id,value) { var $x = ["SetValue",1,id,value]; $x.__enum__ = cdb_jq_Answer; return $x; };
cdb_jq_Answer.Done = function(eid) { var $x = ["Done",2,eid]; $x.__enum__ = cdb_jq_Answer; return $x; };
